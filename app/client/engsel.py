...
def _timeout_for_path(path: str) -> tuple[int, int]:
    # Fast-read endpoints should fail fast to keep UI responsive.
    fast_paths = (
...
def send_api_request(
    ...
    try:
        resp = _SESSION.request(method.upper(), url, headers=headers, data=json.dumps(body), timeout=timeout)
    finally:
        duration_ms = int((time.perf_counter() - start) * 1000)
        logger.info("api_request path=%s method=%s duration_ms=%s", path, method.upper(), duration_ms)
